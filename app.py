import streamlit as st import pandas as pd import requests from langchain_groq import ChatGroq from langchain_core.messages import AIMessage, HumanMessage from langchain_core.tools import tool from langgraph.graph import StateGraph, END from typing import TypedDict, List import os # ------------------------------- # 1️⃣ LOAD DATASET FROM GITHUB # ------------------------------- DATA_URL = "https://raw.githubusercontent.com/Tanish-analyst/ecommerce-analytics-agent/main/ecommerce_sales_dataset.xlsx" langchain_tracing = st.secrets["LANGCHAIN_TRACING_V2"] langsmith_project = st.secrets["LANGSMITH_PROJECT"] langsmith_api_key = st.secrets["LANGSMITH_API_KEY"] @st.cache_data def load_data(): df = pd.read_excel(DATA_URL) df["order_date"] = pd.to_datetime(df["order_date"]) return df df = load_data() st.success("Dataset loaded successfully!") st.dataframe(df.head()) # Make global df accessible global_df = df def clean_llm_code(text: str) -> str: """Extract only raw Python code from LLM output.""" import re # Remove markdown
blocks
    text = text.replace("
python", "").replace("
", "")
    ...
    with st.spinner("Thinking..."):
        ...
        for msg in reversed(result["messages"]):
            if isinstance(msg, AIMessage):
                ...
                break. first tell what will you do. then after my permission
